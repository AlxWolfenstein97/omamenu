...
Item {
  ...
  // Highlighted rows marquee their label/detail when the text would
  // otherwise be cut off by an ellipsis (apps, fonts, plugin ids, etc.).
  component ScrollingLabel: Item {
    id: scrollRoot
    ...
    readonly property bool overflowing: fullWidth > width + 0.5
    readonly property bool scrolling: active && overflowing && visible && width > 0
    ...
    TextMetrics {
      id: metrics
      text: label.text
      font.family: scrollRoot.fontFamily
      font.pixelSize: scrollRoot.fontPixelSize
      font.weight: scrollRoot.fontWeight
    }
    ...
    Text {
      ...
      font.pixelSize: scrollRoot.fontPixelSize
      font.weight: scrollRoot.fontWeight
      opacity: scrollRoot.textOpacity
      width: scrollRoot.scrolling ? scrollRoot.fullWidth : scrollRoot.width
      elide: scrollRoot.scrolling ? Text.ElideNone : Text.ElideRight
      x: 0
    }

    SequentialAnimation {
      id: scrollAnim
      running: scrollRoot.scrolling
      ...
      NumberAnimation {
        target: label
        property: "x"
        to: -Math.max(0, scrollRoot.fullWidth - scrollRoot.width)
        duration: Math.max(1600, Math.max(0, scrollRoot.fullWidth - scrollRoot.width) * 28)
    ...
    onWidthChanged: if (!scrolling) label.x = 0
    onTextChanged: {
      label.x = 0
    ...
    onActiveChanged: {
      label.x = 0
      if (scrolling) scrollAnim.restart()
    }
  }
  ...
  // Used when appLibrary is unavailable (third-party menu clone): same hide
  ...
  // The apps provider is QML-native: prefer the injected AppLibrary when the
  // host wires shell.appLibrary (first-party). Cloned third-party menus often
  // get a null shell, so fall back to DesktopEntries + local AppSearch with
  // the same hide filters AppLibrary applies.
  ...
  function isHiddenEntry(entry) {
    var id = root.normalizeDesktopId((entry && entry.id) || "")
    return id.length > 0 && (root.configuredHiddenEntryIds[id] === true || root.desktopHiddenEntryIds[id] === true)
  }
  ...
  function mergeAppRows() {
    var rows = root.appLibrary
      ? root.appLibrary.sortedEntries("")
      : AppSearch.sortedEntries(DesktopEntries.applications.values || [], "", function(entry) {
    ...
    for (var j = 0; j < rows.length; j++) {
      ...
      if (!appId) continue
      var subtext = root.appLibrary
        ? root.appLibrary.entrySubtext(entry)
        : AppSearch.entrySubtext(entry)
      var label = root.appLibrary
        ? root.appLibrary.entryName(entry)
        : AppSearch.entryName(entry)
  ...
  function launchApp(appId, label) {
    ...
    Util.execDetached("uwsm-app -- gtk-launch " + Util.shellQuote(String(appId || "") + ".desktop"))
  }

  function removeApp(appId, label) {
    if (root.appLibrary) {
      root.appLibrary.remove(appId, label)
      return
    }
    Util.execDetached(Util.shellQuote(root.omarchyPath + "/bin/omarchy-remove-launcher-entry")
      + " " + Util.shellQuote(String(appId || ""))
      + " " + Util.shellQuote(String(label || appId || "")))
  }
  ...
  Process {
    ...
    command: ["bash", "-c", Util.shellQuote(root.omarchyPath + "/shell/services/hidden-entries.sh") + " "
      + Util.shellQuote([Quickshell.env("XDG_CURRENT_DESKTOP"), Quickshell.env("XDG_SESSION_DESKTOP"), Quickshell.env("DESKTOP_SESSION")].filter(function(v) { return String(v || "").length > 0 }).join(":"))]
  ...
  Component.onCompleted: {
    if (!root.appLibrary) hiddenEntryScan.running = true
  }
            ...
            delegate: BorderSurface {
              ...
              Column {
                ...
                ScrollingLabel {
                  ...
                  color: row.hasCursor ? root.selectedText : root.foreground
                  fontFamily: root.fontFamily
                  fontPixelSize: Style.font.heading
